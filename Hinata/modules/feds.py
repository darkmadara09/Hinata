...
import Hinata.modules.sql.feds_sql as sql
from Hinata import (
    EVENT_LOGS,
    GENINS,
    LOGGER,
...
from Hinata.modules.helper_funcs.alternate import send_message
from Hinata.modules.helper_funcs.chat_status import is_user_admin
from Hinata.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from Hinata.modules.helper_funcs.string_handling import markdown_parser
...
def del_fed(update: Update, context: CallbackContext):
    ...
    if args:
        ...
        if getinfo is False:
            ...
            return
        if int(getinfo["owner"]) == int(user.id) or int(user.id) == NARUTO_ID:
            fed_id = is_fed_id
        else:
...
def leave_fed(update: Update, context: CallbackContext):
    ...
    if getuser in "creator" or user.id in JONINS:
...
def user_join_fed(update: Update, context: CallbackContext):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in JONINS:
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in JONINS:
        message.reply_text("JONINS cannot be fed banned!")
        return

    if int(user_id) in GENINS:
        message.reply_text("GENINS cannot be fed banned!")
        return

    if int(user_id) in ACADEMY_USERS:
        message.reply_text("ACADEMY_USERS cannot be fed banned!")
        return
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "Hinata_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="Hinata_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "Hinata_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="Hinata_fbanned_users.csv",
            ...
            with BytesIO() as file:
                ...
                for x in splitting:
                    ...
                    if is_user_fed_admin(fed_id, import_userid) is True:
                        failed += 1
                        continue
                    if str(import_userid) == str(NARUTO_ID):
                        failed += 1
                        continue
                    if int(import_userid) in JONINS:
                        failed += 1
                        continue
                    if int(import_userid) in GENINS:
                        failed += 1
                        continue
                    if int(import_userid) in ACADEMY_USERS:
                        failed += 1
                        continue
...
def is_user_fed_admin(fed_id, user_id):
    ...
    if fed_admins is False:
        return False
    if int(user_id) in fed_admins or int(user_id) == NARUTO_ID:
        return True
    else:
        return False
...
def is_user_fed_owner(fed_id, user_id):
    ...
    if str(user_id) == getfedowner or int(user_id) == NARUTO_ID:
        return True
    else:
        return False
