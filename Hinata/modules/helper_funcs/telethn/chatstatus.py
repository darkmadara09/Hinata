...
from Hinata import JONINS
from Hinata.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
        ...
        if user_id == user.id or user_id in JONINS:
            status = True
            break
    return status
...
async def Hinata_is_admin(chat_id: int):
    status = False
    Hinata = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id,
        filter=ChannelParticipantsAdmins,
    ):
        if Hinata.id == user.id:
            status = True
            break
    return status
