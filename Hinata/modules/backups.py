...
# from Hinata.modules.sql import warns_sql as warnssql
import Hinata.modules.sql.blacklist_sql as blacklistsql

# from Hinata.modules.sql import cust_filters_sql as filtersql
# import Hinata.modules.sql.welcome_sql as welcsql
import Hinata.modules.sql.locks_sql as locksql
import Hinata.modules.sql.notes_sql as sql

# from Hinata.modules.rules import get_rules
import Hinata.modules.sql.rules_sql as rulessql
from Hinata import JOIN_LOGGER, LOGGER, NARUTO_ID, SUPPORT_CHAT, dispatcher
from Hinata.__main__ import DATA_IMPORT
from Hinata.modules.connection import connected
from Hinata.modules.helper_funcs.alternate import typing_action
from Hinata.modules.helper_funcs.chat_status import user_admin
from Hinata.modules.sql import disable_sql as disabledsql
...
def export_data(update, context):
    ...
    with open("Nezuko{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("Nezuko{}.backup".format(chat_id), "rb"),
        caption="💾*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `Nezuko-Backup` was specially made for notes 📚.".format(
            chat.title, chat_id, tgl
        ),
        timeout=360,
    ...
    os.remove("Nezuko{}.backup".format(chat_id))  # Cleaning file
