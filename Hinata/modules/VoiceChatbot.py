...
from Hinata import pbot as LYCIA
...
@LYCIA.on_message(filters.command("Hinata"))
async def Lycia(_, message):
    ...
    m = await message.reply_text("Hinata Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=Hinata&ownername=@Simpleboy787&user=1"
        )
        chatbot = L["message"]
