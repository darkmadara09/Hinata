...
from Hinata.modules.helper_funcs.misc import send_to_list
from Hinata.modules.sql.users_sql import get_user_com_chats
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in GENINS:
        message.reply_text("That's a genin! They cannot be banned!")
        return
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, JONINS + CHUNINS, log_message, html=True)
...
def ungban(update: Update, context: CallbackContext):
        ...
        try:
            ...
            if excp.message in UNGBAN_ERRORS:
                ...
                if EVENT_LOGS:
                    ...
                    bot.send_message(
                        NARUTO_ID, f"Could not un-gban due to: {excp.message}"
                    )
                return
        except TelegramError:
            pass
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, JONINS + CHUNINS, "un-gban complete!")
...
def __user_info__(user_id):
    ...
    if user_id == dispatcher.bot.id:
        return ""
    if int(user_id) in JONINS + GENINS + ACADEMY_USERS:
        return ""
    if is_gbanned:
