...
import Hinata.modules.sql.vanitas_sql as sql
...
from Hinata.modules.helper_funcs.chat_status import user_admin, user_admin_no_reply
from Hinata import dispatcher, updater, SUPPORT_CHAT, SYL
    ...
    if match:
        ...
        if is_vanitas:
            ...
            update.effective_message.reply_text(
                "Vanitas System Disable by {}".format(
...
def vanitasadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_vanitas:
            ...
            update.effective_message.reply_text(
                "Vanitas System Enable by {}".format(
                    mention_html(user.id, user.first_name)
                ),
                parse_mode=ParseMode.HTML,
            )
...
def vantiasban(update: Update, context: CallbackContext):
    ...
    if not is_vanitas:
        set_vanitas = sql.set_vanitas(chat_id)
        if set_vanitas:       
            update.effective_message.reply_text(
                 "Vanitas System Enable by {}".format(
                    mention_html(user.id, user.first_name)
        ...
        rm_vanitas = sql.rm_vanitas(chat_id)
        if rm_vanitas:
            update.effective_message.reply_text(
                 "Vanitas System Disable by {}".format(
                    mention_html(user.id, user.first_name)
    ...
    if x:
        ...
        if should_message:
            ...
            update.effective_message.reply_video(
                ...
                f"<b>Mode</b>: Enable Modes Using /vanitas Disable mode or Change mode\n"
...
VANITASBAN_HANDLER = CommandHandler("vanitasban", vanitasban, run_async=True)
...
dispatcher.add_handler(VANITASBAN_HANDLER)
